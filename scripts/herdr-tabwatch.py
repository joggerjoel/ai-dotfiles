...
def local_names():
    ...
    #
    # Absolute, because the plists generated by herdr-node.sh put neither
    # /usr/sbin nor /sbin on PATH. A probe that cannot run does not raise. It
    # returns one name fewer, and an alias pointing at that name stops looking
    # like ourselves. HERDR_TABWATCH_SCUTIL lets the suite supply a synthetic
    # identity; nothing sets it in production.
    scutil = os.environ.get("HERDR_TABWATCH_SCUTIL", "/usr/sbin/scutil")
    try:
