...
3. **Herdr Runtime Interface**:
   ...
   - **Capabilities depended on**, probed against **herdr 0.9.0** (client and server, protocol 22, socket `~/.config/herdr/herdr.sock`) on 2026-09-12. Every row below was run. `✓` means the real invocation is in the Real column and works; `✗` means the assumed invocation does not exist and the Real column is the replacement.
     ...
     **What "never repeat the sentinel in a prompt" actually bans.** Read as banning the bare
     word, the rule is unsatisfiable: section 4 rule 6 makes `MACHINE.md` the channel by which an
     agent learns what to print, and if no prompt and no injected file may contain the literal,
     nothing can teach it. The nonce resolves this, and it is the second job the nonce does. The
     supervisor matches `MC-DONE <nonce>` and `MC-EXIT <nonce> <n>`, so what a prompt must never
     carry is the **pair**. The bare literal in `MACHINE.md` is unmatchable because the file
     carries no nonce, and an echo of it therefore cannot satisfy any live match.
     `herdr_master/herdr.py` enforces this on every `agent prompt` and `pane send-text` rather
...
- **Rules**:
  ...
  6. Signal completion by printing `MC-DONE` followed by a space and the nonce supplied
     with the task. The literal lives here and in the supervisor; a prompt supplies only
     the nonce, never the pair (§2.3).
```
...
- **2026-09-13 — herdr client layer, and the sentinel rule made satisfiable.** `herdr_master/herdr.py` wraps only the surfaces 2.3 verified, typed, with the text-returning commands separated from the JSON-returning ones. Building it exposed a rule that could not be obeyed as written. "The literal sentinel is never repeated in an injected prompt", read as banning the bare word, leaves no channel by which an agent can learn what to print, since section 4 rule 6 names `MACHINE.md` as that channel. The nonce resolves it: the supervisor matches the sentinel *with a nonce beside it*, so the pair is what a prompt must never carry, and a bare literal in `MACHINE.md` is unmatchable. The client enforces this on every prompt and send-text, and a test asserts the guard and the supervisor's own matcher agree about what is matchable.
