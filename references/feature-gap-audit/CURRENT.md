...
| P3       | Crew/role registry and charters    | `.agents/skills/secondmate-provisioning/`, `data/secondmates.md`, `bin/fm-secondmate-*.sh`                                                                                                             | Structured, auditable persistent named agents with charter and scope. Relevant only if the crew layer is actually adopted.                                                                                                                                                                                             |

Integration gap in its own right: we install a crew orchestrator on macstudio and never
configure it. Either wire it up or stop provisioning it — paying the install cost for an
unused clone is the worst of both.
