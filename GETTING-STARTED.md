...
`superpowers`, `feature-dev`, `code-review`, `pr-review-toolkit`, `code-simplifier`, `commit-commands`, `frontend-design`, `ui-ux-pro-max`, `agent-browser`, `claude-mem`, `codex`, `andrej-karpathy-skills`, `skill-creator`, `typescript-lsp`, `security-guidance`, `serena`, `supabase`, `stripe`, `pg`, `n8n-mcp-skills`, `autoresearch`, `ralph-loop`, `plugin-dev`, `hookify`, `agent-sdk-dev`, `claude-md-management`, `claude-code-setup`, `chrome-devtools-mcp`, `elements-of-style`, `learning-output-style`, `caveman`, `static-analysis`, `variant-analysis`, `audit-context-building`.

`settings.json` marks all 34 as `enabled`, but `bootstrap-plugins.sh` only auto-installs the **15 core** (the shipping engine above); the other 19 (`supabase`, `stripe`, `pg`, `autoresearch`, `n8n-mcp-skills`, `ralph-loop`, `serena`, `chrome-devtools-mcp`, `plugin-dev`, `hookify`, `agent-sdk-dev`, `claude-md-management`, `claude-code-setup`, `elements-of-style`, `learning-output-style`, `caveman`, `static-analysis`, `variant-analysis`, `audit-context-building`) are the **opt-in groups** from Part 2 — pre-enabled in the committed desktop profile but only installed if you opt in during setup. An enabled-but-not-installed plugin is simply inert until you install it (`claude plugin install <plugin>@<marketplace>`). The 15/19 split mirrors the bootstrap `CORE` vs `OPT_*` arrays exactly. Two more marketplaces are declared but their plugins left off by default: `context-mode` (hooks every Bash/Read/Grep call and overlaps `claude-mem`) and `firecrawl` (needs a Firecrawl API key). `caveman` is enabled but starts every session off — `CAVEMAN_DEFAULT_MODE=off` in the profile's `env` — so `/caveman` is an on-demand switch, never a default.

Local skills shipped by this repo (`skills/`): `getting-started`, `color-strategy`, `design-taste`, `react-bits`, `uiverse`, `verify`, `verify-ui`, `verify-tui`, `test-and-fix`, `review-changes`, `explore-plan-code-test`, `first-principles`, `feature-gap-audit`, `quick-commit`, `responsive-audit`, `humanizer`, `reflection`, `context-dump`, `scrapegraph`, plus six vendored from third-party skill repos by `scripts/vendor-agent-skills.sh` (pinned in each folder's `.upstream`): `handoff`, `grill-me` (Matt Pocock), `web-design-guidelines`, `vercel-react-best-practices`, `vercel-composition-patterns` (Vercel Labs), `webapp-testing` (Anthropic; needs Playwright). `setup.sh` also runs `firecrawl init`, which installs the 28 `firecrawl-*` skills into `~/.agents/skills` and links them into the Claude, Codex, Gemini and Cursor skill dirs; `just fleet-firecrawl` does the same on every fleet host, plus the API key. It also installs `@playwright/cli` and its `playwright-cli` skill globally, into both `~/.claude/skills` and `~/.agents/skills`; browsers are not included. On a Mac it also installs [Orca](https://onorca.dev/) (Stably AI's agent-orchestration IDE and its `orca` CLI) as a Homebrew cask, adopting an existing DMG install, and installs Orca's 8 bundled agent skills (`orca-cli`, `orchestration`, `computer-use`, ...) into `~/.agents/skills` with links into every harness; `just fleet-orca` does the same on every fleet Mac.
