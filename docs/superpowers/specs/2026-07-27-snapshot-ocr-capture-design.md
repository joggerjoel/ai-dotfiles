...
Right now the only way to teach the bot about Jo is to type it. That is a hard
...
context for extraction — "this is Jo's new boss" alongside the image tells the
...
> as shared; Jo can see this. Reply `private` to restrict it.
