...
```
PRMM_DATABASE_URL=postgres://voice:PASSWORD@<DB_HOST>:5433/voice?options=-csearch_path%3Dprmm,public
```
  ...
  messages: [
    {
      ...
      text: "User B and I met at the hacker house",
...
```json
{
  ...
  "messages": [
    {
      "from": "+15555550100",
      "text": "User B and I met at the hacker house in 2019"
    },
...
describe("parseIntent", () => {
  it("recognises where-met", () => {
    expect(parseIntent("where did I meet User B?")).toEqual({
      kind: "where-met",
      subject: "User B",
    });
  });
...
describe("answerQuestion", () => {
  it("answers where-met from first_met_context", async () => {
    const reply = await answerQuestion("where did I meet User B?", 1, deps());
...
```
@calvin where did I meet User B?
```
...
describe("private memories about the other participant", () => {
  const joPerson = {
    id: 99, canonicalName: "User B", aliases: [], company: null,
  ...
  it("stores and announces who cannot see it", async () => {
    ...
    const out = await runCapture(
      batch("User B has been stressed about work", { kind: "dm", userId: 1 }, [1]),
      deps({
        repo: r,
        extractor: extractorOf({
          people: [person("User B")],
          memories: [memory("User B", "has been stressed about work", 0.9)],
        }),
    ...
    expect(out.announcements.join(" ")).toMatch(/privately about User B/i);
...
- [ ] A message naming User B produces a person row for User B
...
- [ ] `@calvin where did I meet User B?` answers and is never captured
